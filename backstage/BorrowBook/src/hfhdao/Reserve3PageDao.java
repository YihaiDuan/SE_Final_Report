...
public class Reserve3PageDao {
	public Page<OnlineOrder> findByPage(int pc) 
	{
		...
		list=session.createQuery("from OnlineOrder where status=1 order by orderdate asc ").list();
		if(list!=null)
		{		
			all=list.size();
		}
       ...
       list2=session.createQuery("from OnlineOrder  where status=1 order by orderdate asc").
